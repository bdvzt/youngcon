...
final class MockUsersRepository: UsersRepositoryProtocol {
    ...
    var mockProfile = UserProfile.placeholder
...
final class MockAchievementsRepository: AchievementsRepositoryProtocol {
    ...
    var mockAchievements: [Achievement] = [
        Achievement(id: "ach-1", name: "Тест 1", description: "Desc 1", icon: "https://test.com/1.png", color: "#FFFFFF"),
        Achievement(id: "ach-2", name: "Тест 2", description: "Desc 2", icon: "https://test.com/2.png", color: "#000000"),
    ]
...
final class BadgeViewModelTests: XCTestCase {
    ...
    func testLoadData_Success_StateUpdatesCorrectly() async {
        mockUsersRepo.mockUnlockedAchievements = [
            Achievement(id: "ach-1", name: "", description: "", icon: nil, color: ""),
        ]

        await viewModel.loadData()

        XCTAssertFalse(viewModel.isLoading, "isLoading должен стать false после завершения")
        XCTAssertEqual(viewModel.profile?.id, "00000000-0000-0000-0000-000000000000", "Профиль должен быть установлен")
